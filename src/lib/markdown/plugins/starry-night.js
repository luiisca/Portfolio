 ...
 * @import {Grammar, Options as StarryNightOptions} from '@wooorm/starry-night'
 */

/**
 * @typedef {StarryNightOptions & {
 *   allowMissingScopes?: boolean | null | undefined, // Do not warn for missing scopes (default: `false`).
 *   grammars?: ReadonlyArray<Grammar> | null | undefined, // Grammars to support (default: `common`).
 *   plainText?: ReadonlyArray<string> | null | undefined // List of language names to not highlight (default: `[]`).
 * }} Options
 */

/**
 * @typedef {[string, number]} DistanceTuple
 */
				...
				if (languageClass) {
					...
					if (plainText.includes(language)) {
						...
						node.children = /** @type {Array<ElementContent>} */ ([
							{
								type: 'raw',
								value: `{@html \`${escape_svelty(toHtml(fragment))}\`}`
							}
						]);
					} else {
						let reason =
